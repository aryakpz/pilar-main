import { LanguageType } from "../types/enum"

type SelectProps={
    onSelectChange:(value:LanguageType)=>void
}
        ...
        <div className="w-1/2 flex flex-col">
            ...
            <div className="flex relative w-full">
                <select className="w-full border bg-transparent rounded-md text-sm p-2 appearance-none bg-down-arrow bg-no-repeat bg-[length:auto_1.4rem] pr-5 bg-arrow-position focus:ring-blue-500"
                    onChange={(e) => onSelectChange(e.target.value as LanguageType)}>
                    <option value={LanguageType.ENG}>{LanguageType.ENG}</option>
                    <option value={LanguageType.DUT}>{LanguageType.DUT}</option>
                </select>
            </div>
